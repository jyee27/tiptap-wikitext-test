...
class WikiTextSerializerState {
   ...
   render(node: Node): void {
      let f = this.nodes[node.type.name];
      if (!f) {
         console.log('no function for ' + node.type.name);
         return;
      }
      f(this, node);
   }
   ...
   closeMarks(marks : ReadonlyArray<Mark>) {
      this.out += marks.reduceRight((carry, mark) => {
         ...
         if (!close) {
            console.log('mark not found: ' + mark.type.name);
            return carry;
         }
...
const serializer = new WikiTextSerializer({
   ...
   hardBreak(state: WikiTextSerializerState, node: Node): void {
      state.append("[br]\n")
   },
   ...
   codeBlock(state: WikiTextSerializerState, node: Node): void {
      state.append("[code]\n")
      state.inline(node)
      state.append("\n[/code]\n")
   },
   ...
   orderedList(state: WikiTextSerializerState, node: Node): void {
      state.renderList(node, '#')
   },

   bulletList(state: WikiTextSerializerState, node: Node): void {
      state.renderList(node, '*')
   }
}, {
   // Marks
